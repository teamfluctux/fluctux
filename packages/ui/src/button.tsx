...
const buttonVariants = cva(
  "focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 rounded-lg border border-transparent bg-clip-padding text-sm font-medium focus-visible:ring-3 aria-invalid:ring-3 [&_svg:not([class*='size-'])]:size-4 group/button inline-flex shrink-0 items-center justify-center whitespace-nowrap transition-all outline-none select-none disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-primary-color text-text-color_default_white hover:bg-primary-color-hover [a]:hover:bg-primary/80 disabled:text-text-color_default_white-2 disabled:cursor-not-allowed! disabled:bg-primary-color-disabled",
        outline:
          "border-border bg-background hover:bg-muted hover:text-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 aria-expanded:bg-muted aria-expanded:text-foreground",
        secondary:
          "bg-background-color_800C text-text-color_1 hover:bg-background-color_750C disabled:bg-background-color_850C isabled:text-text-color_3 isabled:hover:bg-background-color_850C aria-expanded:bg-secondary aria-expanded:text-secondary-foreground",
        ghost:
...
function Button({
  ...
  size = "default",
  asChild = false,
  showClickOutlineEffect = false,
  ...props
}: React.ComponentProps<"button"> &
  VariantProps<typeof buttonVariants> & {
    asChild?: boolean;
    showClickOutlineEffect?: boolean;
  }) {
