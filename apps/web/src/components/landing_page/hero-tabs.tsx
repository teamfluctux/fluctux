    ...
    <div className="w-full px-[1px]">
      <div>
        <div className=" left-1/2 -translate-x-1/2 w-fit relative h-[35px] pb-[0.4rem] flex justify-center items-center">
          ...
          <div
            ref={chromeTabRef}
            className="w-[100px] left-0 h-full transition-all duration-500 absolute bg-gradient-to-t from-background-color_850C via-background-color_850C to-background-color_800C bottom-0 -z-10 rounded-t-[10px] chrome_tab "
          ></div>
          ...
          {HERO_WORKSPACE_SLIDER_TYPE.map((item, i) => {
            ...
            return (
              <button
                ...
                className={`transition-colors group fx-flex-center gap-1  rounded h-full px-3 text-workspace_2 font-medium  ${slideIndex === i ? "text-text-color_1 bg-transparent" : "text-text-color_2 hover:text-text-color_1"}`}
              >
                <div className="text-text-color_1">
                  <Icon
                    size={16}
                    className={` transition-colors duration-300 ${slideIndex === i ? "text-text-color_1" : "text-text-svg_default group-hover:text-text-color_1"}`}
                  />
                </div>
      ...
      <div className="w-full h-[650px] p-2  rounded-[25px] bg-background-color_850C hero_workspace_box relative">
        ...
        <div className="w-full h-full bg-background-color_925C overflow-hidden rounded-[20px] border border-border-color_1">
          <Swiper
            ...
            effect={"fade"}
          >
            <SwiperSlide className="bg-background-color_925C">Slide 1</SwiperSlide>
