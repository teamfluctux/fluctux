...
const DynamicDocSidebar = dynamic<DocSidebarPropsType>(
    () => import("./doc-sidebar"),
    {
        ssr: false,
        loading: () => <aside
            className={`w-[250px] h-screen sticky top-0 fx-primary-bg flex-shrink-0 doc-aside-nav doc-aside-nav-loader transition-all duration-150 ease-out`}
        >
...
export const DocOnDemandSidebar = ({ docType, data }: DocSidebarPropsType) => {
    ...
    return <>
        {
            data && <DynamicDocSidebar docType={docType} data={data} />
        }
    </>
