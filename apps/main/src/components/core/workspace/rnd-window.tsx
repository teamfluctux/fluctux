...
import { TabsRndType, TaskbarCategoriesType } from '@fluctux/types'
import { cn, LUCIDE_WORKSPACE_ICON_SIZE } from '@fluctux/ui'
import { Maximize, Minimize, Minus, X } from 'lucide-react'
import React, { useState } from 'react'
import { Rnd } from 'react-rnd'

export const RndWindow = ({ tab, tabType, category, children, ...props }: {
    tab: TabsRndType,
    tabType: TaskbarCategoriesType,
    category: string,
    ...
    return <>
        ...
        <Rnd
            onMouseDown={() => updateTabInCategory(tabType, tab.id!, { isActive: true })}
            ...
            onResize={(e, direction, ref, delta, pos) => {
                setIsDragStart(true)
                updateTabInCategory(tabType, tab.id!, {
            ...
            onDrag={(e, d) => {
                ...
                updateTabInCategory(tabType, tab.id!, { isActive: true })
            ...
            onDragStop={(e, d) => {
                updateTabInCategory(tabType, tab.id!, { position: { x: d.x, y: d.y } });
                setIsDragStart(false);
                setEnabledFullWindow(false)
                setEnabledBottomWindow(false)
                setEnabledLeftWindow(false)
        ...
        >
            <div className="h-[30px] border-b border-border-color_2 fx-flex-between-ic pl-2 pr-1 backdrop-blur-lg drag-handle">
                ...
                <div className="fx-flex-cr gap-2">
                    ...
                    <span className="hover:bg-background-color_5 p-[2px] rounded-tiny cursor-pointer"  >
                        {
                            ...
                            <Maximize onClick={(e) => {
                                e.preventDefault()
                                handleMaxMinTabSize(tab.id!, tabType)
                    ...
                    <span onClick={() => handleCloseTab(tabType, tab.id!)} className="p-[2px] cursor-pointer rounded-tiny hover:bg-red-600">
