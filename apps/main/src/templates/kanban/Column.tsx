...
export const KanbanColumn = (props: KanbanColumnPropsType) => {
  ...
  const {setNodeRef, isDragging, listeners, attributes, transform, transition} = useSortable({
    id: column.id,
    data: {
      type: "column",
      column,
  ...
  if(isDragging) {
    return <div ref={setNodeRef} style={style} className="w-full min-w-[330px] max-w-[330px]  h-full">
  ...
  return  <div ref={setNodeRef} style={style} className="w-full min-w-[320px] max-w-[320px]  h-full">
              <div className="h-[40px] flex justify-between items-center group relative overflow-hidden">
                <div className="flex justify-start items-center gap-3 w-fit">
                  <FxButton
