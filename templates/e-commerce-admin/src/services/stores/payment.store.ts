...
class PaymentStore {
    ...
    setSelectedPaymentProviders(data: SelectedPaymentProvidersType) {
        if (this.selectedPaymentProviders.find(p => p.value == data.value)) {
            this.selectedPaymentProviders = this.selectedPaymentProviders.filter(p => p.value != data.value)
            return
        }
        this.selectedPaymentProviders = [...this.selectedPaymentProviders, { ...data }]
    }
